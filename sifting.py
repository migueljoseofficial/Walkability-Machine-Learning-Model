...
def remove_irrelevant_rows(
    ...
    longitude_min: float = -86.243803,
    longitude_max: float = -86.253347
) -> pd.DataFrame:
    filtered_data = heart_data[
        (heart_data['Latitude'] >= latitude_min) &
        (heart_data['Latitude'] <= latitude_max) &
        (heart_data['Longitude'] >= longitude_min) &
        (heart_data['Longitude'] <= longitude_max)
    ]
...
heart_file = remove_irrelevant_rows(dirty_heart_file, latitude_min= 41.677476,
